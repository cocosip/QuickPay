...
namespace QuickPay.UnitTest.Infrastructure.Util
{
    public class RequestReflectUtilTest
    {
        ...
        {
            var request = new AppUnifiedOrderRequest("hello", "123456", 1, "");
            var payData = RequestReflectUtil.ToPayData(request);
            Assert.Equal("hello", payData.GetValue("body"));
            Assert.Equal("123456", payData.GetValue("out_trade_no"));
            Assert.Equal(1, payData.GetValue("total_fee"));
        }
        ...
        public void ToResponse()
        {
            ...
            var request = (AppUnifiedOrderRequest)RequestReflectUtil.ToResponse(payData, typeof(AppUnifiedOrderRequest));
